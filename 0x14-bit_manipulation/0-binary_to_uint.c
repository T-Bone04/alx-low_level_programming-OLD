 ...
 * or 0 if b contains invalid characters
 */
unsigned int binary_to_uint(const char *b)
{
	...
	while (b[index] != '\0') /* Loop through each character in b */
		/* until the null terminator is reached */
	{
		if (b[index] != '0' && b[index] != '1')
		/* Check if the current character is not a valid binary digit */
			return (0);
		/* Return 0 if the current character is not a valid binary digit */
		...
		index++; /* Increment the index variable to */
